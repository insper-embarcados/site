...
::: tip Rule 4.3
Utilizar o `vTaskDelay` ao querer suspender temporariamente uma task. 
:::

O `vTaskDelay` sinaliza ao kernel de que a `task` não quer exectuar por `x` ticks, o que a faz entrar em um estado de não execucão, permitindo que outras `tasks` executem. 

Essa regra possui uma **exceção**, que é quando precisamos gerar pequenos delays na ordem de **microssegundos**; nesse caso**,** a melhor maneira de fazer é usando delays de software como o `sleep_us()`. O embedded-check foi configurado para liberar pequenos delays com `sleep` sem acusar erros.
